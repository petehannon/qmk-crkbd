...
static void render_logo(void) {
	...
	if 		(layer_state_is(NUMB_LAYER|SYMB_LAYER)) { oled_write_P(PSTR(" func"), false); }
	else if (layer_state_is(SYMB_LAYER))   			{ oled_write_P(PSTR(" symb"), false); }
	else if (layer_state_is(NUMB_LAYER))   			{ oled_write_P(PSTR(" numb"), false); }
	else    										{ oled_write_P(PSTR(" base"), false); }
}
